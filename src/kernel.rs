...
impl<T: Float> Kernel<T> for Pow2<T> {
    ...
    fn dddf(&self, _: T) -> T { T::zero() }
    fn ddddf(&self, _: T) -> T { T::zero() }
    fn df_l(&self, _: T) -> T { T::from(2.0).unwrap() }
    fn g(&self, _: T) -> T { T::zero() }
    fn g_l(&self, _: T) -> T { T::zero() }
    fn h(&self, _: T, _: T) -> T { T::zero() }
}
...
impl<T: Float> Kernel<T> for Pow3<T> {
    ...
    fn dddf(&self, _: T) -> T { T::from(6.0).unwrap() }
    fn ddddf(&self, _: T) -> T { T::zero() }
    fn df_l(&self, x: T) -> T { T::from(3.0).unwrap()*x }

    // WARNING: This function does not go to zero as x -> 0
    fn g(&self, _: T) -> T { T::from(3.0).unwrap() }

    // WARNING: This function doesn't exist at x = 0.
    fn g_l(&self, x: T) -> T { T::from(3.0).unwrap()/x }

    // WARNING: This function doesn't exist at x = 0
    fn h(&self, x: T, a: T) -> T { T::from(3.0).unwrap()*(T::from(2.0).unwrap() - a)/x }
}
...
impl<T: Float> Kernel<T> for Pow4<T> {
    ...
    fn dddf(&self, x: T) -> T { T::from(24.0).unwrap()*x }
    fn ddddf(&self, _: T) -> T { T::from(24.0).unwrap() }
    fn df_l(&self, x: T) -> T { T::from(4.0).unwrap()*x*x }
    fn g(&self, x: T) -> T { T::from(8.0).unwrap()*x }
    fn g_l(&self, _: T) -> T { T::from(8.0).unwrap() }
    fn h(&self, _: T, a: T) -> T { T::from(24.0).unwrap() - a*T::from(8.0).unwrap() }
}
...
impl<T: Float> Kernel<T> for Pow5<T> {
    ...
    fn dddf(&self, x: T) -> T { T::from(60.0).unwrap()*x*x }
    fn ddddf(&self, x: T) -> T { T::from(120.0).unwrap()*x }
    fn df_l(&self, x: T) -> T { T::from(5.0).unwrap()*x*x*x }
    fn g(&self, x: T) -> T { T::from(15.0).unwrap()*x*x }
    fn g_l(&self, x: T) -> T { T::from(15.0).unwrap()*x }
    fn h(&self, x: T, a: T) -> T { (T::from(60.0).unwrap() - a*T::from(15.0).unwrap())*x }
}
...
impl<T: Float> Kernel<T> for Gauss<T> {
    ...
    fn df_l(&self, x: T) -> T { -(T::from(2.0).unwrap()/self.r) * self.f(x) }
    fn g(&self, x: T) -> T { T::from(4.0).unwrap()*x*self.f(x)/(self.r*self.r) }
    fn g_l(&self, x: T) -> T { T::from(4.0).unwrap()*self.f(x)/(self.r*self.r) }
    fn h(&self, x: T, a: T) -> T {
        let _4 = T::from(4.0).unwrap();
        let _3 = T::from(3.0).unwrap();
        let _2 = T::from(2.0).unwrap();
...
impl<T: Float> Kernel<T> for Csrbf31<T> {
    ...
    fn ddddf(&self, x: T) -> T {
		...
		if x > _1 {
            ...
            T::from(120).unwrap() * (T::from(4).unwrap() * x - T::from(3).unwrap()) / (self.r*self.r*self.r*self.r)
        }
    }
    fn df_l(&self, x: T) -> T {
        let _1 = T::one();
    ...
    fn h(&self, x: T, a: T) -> T {
		...
		if x > _1 {
            ...
            T::from(60).unwrap() * t*(_2 - _4*x - a*t)/(x*self.r*self.r*self.r*self.r)
        }
    }
}
...
impl<T: Float> Kernel<T> for Csrbf42<T> {
    ...
    fn ddddf(&self, x: T) -> T {
		...
		if x > _1 {
            ...
            let t = x - _1;
            T::from(1680).unwrap() * t * t * (_5*x - _3) * (_7*x - _1) / (self.r*self.r*self.r*self.r)
        }
    }
    fn df_l(&self, x: T) -> T {
        let _1 = T::one();
    ...
    fn h(&self, x: T, a: T) -> T {
		...
		if x > _1 {
            ...
            T::from(1680).unwrap()*t*t*t*(_3 - _7*x - a*t)/(self.r*self.r*self.r*self.r)
        }
    }
}
